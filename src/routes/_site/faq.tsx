 ...
 *
 * Price-window hours come from GET /api/catalog (ORDER_EXPIRY_HOURS) so FAQ
 * stays aligned with the invoice deadline without a frontend redeploy.
 */
function buildSections(expiryPhrase: string | null) {
	const priceWindow = expiryPhrase
		? `The price you see stays valid for ${expiryPhrase} from the time you order — your invoice shows the exact time it runs to. Depot prices move through the day, so after that you simply order again at the current price.`
		: "The price you see stays valid for the window shown on your invoice — that page shows the exact time it runs to. Depot prices move through the day, so after that you simply order again at the current price.";

	const unpaidCancel = expiryPhrase
		? `An unpaid order is cancelled automatically after ${expiryPhrase}, and we warn you before that happens. Cancelling costs you nothing — you can simply order again at the current price.`
		: "An unpaid order is cancelled automatically after the deadline on your invoice, and we warn you before that happens. Cancelling costs you nothing — you can simply order again at the current price.";
