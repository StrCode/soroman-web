/**
 * Disabled: this panel showed the customer's permanent Paystack dedicated
 * account on the profile page ("your dedicated funding account"). Paystack
 * DVA funding is off backend-side now — GET /api/customer/profile always
 * returns virtualAccount: null, so `me.virtualAccount()` here would only
 * ever 404 and this panel would sit on its "No account yet" empty state
 * forever, which reads as broken rather than as "not applicable anymore."
 *
 * There's no more self-service, always-available funding account to show:
 * wallet top-ups are staff-recorded manual deposits, and per-order transfer
 * details render on the order/invoice itself (unaffected by this change —
 * see components/order/invoice-step.tsx and orders/depot-order-actions.tsx).
 *
 * Rendering nothing here rather than deleting the file, so the original
 * implementation below is easy to reinstate if Paystack comes back.
 */
